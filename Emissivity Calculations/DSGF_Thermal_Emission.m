...
function [] = DSGF_Thermal_Emission(description, discretization, material, T_equilibrium, epsilon_ref, omega, output, L_char, d, A_c, N_objects);
...
%****************************DISCRETIZATION*******************************%
    ...
    [N_each_object, volume, r_each_object, ind_bulk, delta_V_each_object, L_sub_each_object,origin] = read_discretization_emissivity(discretization, L_char,d);
    ...
    %These deltaV are based on uniformly discretized particles of
    %the same size and shape. If the user modifies the discretization or
    %shape, the user must update the variable delta_V_subvolume
